...
const _DATA_VER = '27';   // bump quand un /data/*.json ou une image de bloc change (force le rechargement)
...
window.DB_READY = Promise.all([
...
]).then(([weapons, armor, items, enemies, perks, npc, ammo, ammoLoot, npcXp, zones, zoneVariations, zoneOccupation, zoneThreat, factions, npcRoles, lootProfiles, weaponMods, armorMods, ency, profiles, buildBlocks, junk, companions, postcards]) => {
  ...
  window.PROFILES = []; window.BUILD_BLOCKS = []; window.BUILD_MATS = {}; window.JUNK = []; window.MAT_LABELS = {}; window.COMPANIONS = []; window.POSTCARDS = [];
});
