...
async function loadGeoJsonLayers() {
  ...
  try {
    ...
    // 1) trait large vif = les rives, 2) trait un peu moins large par-dessus = le lit d'eau,
    // en vert d'eau PLEIN (bien distinct du fond de carte : le fleuve se lit comme rempli).
    L.geoJSON(data, { pane: 'seinePane',
      style: { color: '#4CFF77', weight: 10, opacity: 0.9, lineCap: 'round', lineJoin: 'round', fill: false },
    }).addTo(map);
    L.geoJSON(data, { pane: 'seinePane',
    ...
    L.geoJSON(geoZonesData, {
      ...
      onEachFeature: (f, layer) => {
        ...
        if (('' + (f.properties.Type || '')).toLowerCase().startsWith('crat')) {
          ...
          // Réglages par cratère : échelle (défaut 1) et « stretch » = l'image épouse la
          // boîte englobante du polygone (étirée selon sa forme) au lieu de rester carrée
          const CRATER_IMG_SCALE   = { 'cratère nord': 0.95 };
          const CRATER_IMG_STRETCH = { 'cratère nord': true };
          const nomKey = ('' + (f.properties.Nom || '')).toLowerCase();
          const scale = CRATER_IMG_SCALE[nomKey] || 1.0;
          let halfLng, halfLat;
          if (CRATER_IMG_STRETCH[nomKey]) {
            halfLng = (b.getEast() - b.getWest()) / 2 * scale;
            halfLat = (b.getNorth() - b.getSouth()) / 2 * scale;
          } else {
            halfLng = Math.max((b.getEast() - b.getWest()) / 2, (b.getNorth() - b.getSouth()) / 2 / merc) * scale;
            halfLat = halfLng * merc;
          }
...
// Déblocage auto du Lieu d'encyclopédie — COMMENTÉ (encyclopédie débranchée)
