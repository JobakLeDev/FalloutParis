...
function avancerHorlogeSelection(mins){
  ...
  (selected||[]).forEach(pid => { const p = (tempsData.parties||[]).find(pt => (pt.players||[]).includes(pid)); if(p) parties.add(p); });
  if(!parties.size){ showMsg('Aucun groupe pour la sélection', true); return; }
  parties.forEach(p => { p.minutes = Math.max(0,(p.minutes||0)+mins); });
  saveTemps(); renderParties();
  showMsg(`Horloge avancée de ${mins} min (${parties.size} groupe(s))`);
...
function genDeplacement(){
  const km = Math.max(0, parseFloat(String(document.getElementById('nb-km')?.value||'').replace(',','.')) || 0);
  ...
  if(!events.length){
    ...
    html += `<div class="rencontre-sub" style="margin-top:6px">${events.length} événement(s) sur la route</div>`;
    html += events.map((evt,i)=>{
      const color = (evt.type==='combat'||evt.type==='danger')?'var(--rd)':'var(--am)';
      return `<div class="event-card" style="border-color:${color}">
        <div class="event-type" style="color:${color}">⚡ ${i+1}. ${evt.label.toUpperCase()}</div>
...
// ÉDITEUR POIS & ZONES
// ============================================================
function openMapEditorModal(){
  const mo = document.getElementById('mj-poi-modal'); if(!mo) return;
  mo.classList.add('on');
  _refreshPOIList();
}
...
function openZonesEditorModal(){
  const mo = document.getElementById('mj-zone-modal'); if(!mo) return;
  mo.classList.add('on');
  _refreshZoneList();
}
...
function _refreshPOIList(){
  const list = document.getElementById('poi-list'); if(!list) return;
  const pois = (carteData && carteData.pois) || [];
  list.innerHTML = pois.map((p, i) => `
    <div style="padding:8px;background:#222;border:1px solid var(--b2);display:grid;gap:4px;grid-template-columns:1fr 100px">
      <div><strong>${p.nom || '?'}</strong> (${p.type || 'other'})</div>
      <button class="action-btn dmg" onclick="_deletePOI(${i})" style="padding:4px">🗑 Supprimer</button>
    </div>
  `).join('');
}
...
function _refreshZoneList(){
  ...
  list.innerHTML = zones.map((z, i) => `
    <div style="padding:8px;background:#222;border:1px solid var(--b2);display:grid;gap:4px;grid-template-columns:1fr 100px">
      ...
      <button class="action-btn dmg" onclick="_deleteZone(${i})" style="padding:4px">🗑 Supprimer</button>
    </div>
  `).join('');
}
...
function addNewPOI(){
  ...
  const type = document.getElementById('poi-type')?.value || 'other';
  if(!name) { fpConfirm('Nom obligatoire','err'); return; }

  const newPOI = { id: Date.now(), nom: name, type, lng: 2.35, lat: 48.85, revealedFor: [], icon: 'pin' };
  const pois = [...(carteData?.pois || []), newPOI];

  db.collection('carte').doc(fpCampId()).set({ pois }, { merge: true })
    .then(() => {
      document.getElementById('poi-name').value = '';
      _refreshPOIList();
      fpConfirm('✓ POI ajouté','ok');
    })
    .catch(e => fpConfirm('Erreur : ' + (e.message || e),'err'));
}

function _deletePOI(idx){
  if(!carteData.pois) return;
  const pois = carteData.pois.filter((_, i) => i !== idx);
  db.collection('carte').doc(fpCampId()).set({ pois }, { merge: true })
    .then(() => { _refreshPOIList(); fpConfirm('✓ POI supprimé','ok'); })
    .catch(e => fpConfirm('Erreur : ' + (e.message || e),'err'));
}

function addNewZone(){
  const name = document.getElementById('zone-name')?.value?.trim();
  const type = document.getElementById('zone-type')?.value || 'Danger';
  if(!name) { fpConfirm('Nom obligatoire','err'); return; }
