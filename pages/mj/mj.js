...
// ============================================================
function lancerDes(){
  const nb=Math.min(12,parseInt(document.getElementById('dice-nb').value)||2);
  const f=Math.min(100,parseInt(document.getElementById('dice-faces').value)||20);
  const vals=Array.from({length:nb},()=>Math.floor(Math.random()*f)+1);
  const total=vals.reduce((a,b)=>a+b,0);
  const el=document.getElementById('dice-result');
  el.style.display='block';
  el.innerHTML=`<span style="color:var(--td)">${nb}D${f} → </span>${vals.join(' + ')} <b style="color:var(--am);font-family:'Oswald',sans-serif;font-size:15px"> = ${total}</b>`;
}

function lancerCD(){
  const nb=Math.min(10,parseInt(document.getElementById('dice-nb').value)||2);
  const FACES=['1','2','—','—','⚡','⚡'];
  const vals=Array.from({length:nb},()=>FACES[Math.floor(Math.random()*6)]);
  const dmg=vals.filter(v=>v==='1'||v==='2').reduce((a,v)=>a+parseInt(v),0);
  const ef=vals.filter(v=>v==='⚡').length;
  const el=document.getElementById('dice-result');
  el.style.display='block';
  el.innerHTML=`<span style="color:var(--td)">${nb}DC → </span>${vals.join(' ')} <b style="color:var(--am);font-family:'Oswald',sans-serif;font-size:15px"> = ${dmg} dmg</b>${ef?` + <span style="color:var(--am)">${ef}⚡</span>`:''}`;
}
const ENNEMIS_DB = {
  'Pillard':          {pvd:'1D+2',atq:'3D',rd:0,desc:'Humain hostile basique. STR 5, AGI 5.',xp:25},
  'Pillard Vétéran':  {pvd:'2D+4',atq:'4D',rd:1,desc:'Pillard expérimenté. STR 6, AGI 6. Arme améliorée.',xp:50},
